import * as React from "react";
import SvgIcon from "@material-ui/core/SvgIcon";

const SvgMultipleActionsVideo = (props) =>
  React.createElement(
    SvgIcon,
    props,
    <defs>
      <style>
        {
          ".multiple-actions-video_svg__cls-1{fill:none;stroke:currentColor;stroke-linecap:round;stroke-linejoin:round}"
        }
      </style>
    </defs>,
    <path
      className="multiple-actions-video_svg__cls-1"
      d="M23.5 18.167a.5.5 0 00-.64-.48l-1.36.4V17.5a1 1 0 00-1-1h-6a1 1 0 00-1 1v5a1 1 0 001 1h6a1 1 0 001-1v-.667l1.342.448a.5.5 0 00.658-.475zM7.5 23.5l.5-6h2.5V14a5 5 0 00-10 0v3.5H3l.5 6z"
    />,
    <circle
      className="multiple-actions-video_svg__cls-1"
      cx={5.5}
      cy={4}
      r={3.5}
    />,
    <circle
      className="multiple-actions-video_svg__cls-1"
      cx={14}
      cy={4.5}
      r={3}
      id="multiple-actions-video_svg___Group_"
      data-name="&lt;Group&gt;"
    />,
    <path
      className="multiple-actions-video_svg__cls-1"
      d="M18.7 12.485a5 5 0 00-7.262-2.819"
    />
  );

export default SvgMultipleActionsVideo;
